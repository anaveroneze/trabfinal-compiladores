    ...
    def error(self):
        ...
        for producao in self.producoes:
            ...
            if simb_esq not in self.naoterminais:
                print("ERRO: Símbolo", simb_esq ,"a esquerda da produção", producao, "não está definido nos não terminais.")
                return 1

            for char in simb_dir:
                if (char not in self.terminais) and (char not in self.naoterminais) and (char != '#'):
                    print("ERRO: Símbolo", char ,"a direita da produção", producao, "não está definido.")
                    return 1
        return 0